...
def compute_gradients(model, data, target, output_path="/app/results/grads.json", worker_id=uuid.uuid4(), result_filename = f"res-random-{uuid.uuid4()}.json"):
    ...
    # print("Any NaNs in X_tensor?", torch.isnan(data).any().item())
    # print("Input stats to model - min:", data.min().item(), "max:", data.max().item())
    output = model(data)
    # print(">>> Output stats <<<")
...
if __name__ == "__main__":
    ...
    X = df.drop(columns=["label_encoded", "label", "url", "tld"]).values
